...
<xsl:variable name="pagename" >Педагогическим работникам</xsl:variable>
...
<xsl:template name="Content">
          <div  class="containersved" id="containersved" align="center">
			     ...
			     <xsl:if test="count(EditPasport/Infosec/Educator_DocumentsList/item) > 0">
            <fieldset class='information'>
              <legend><h3>Педагогическим работникам</h3></legend>
                <table border="0" cellspacing="0" cellpadding="3" style='width: 100%;'>
                  <tbody>
                  <xsl:for-each select="EditPasport">
                  <xsl:variable name="text"><xsl:value-of disable-output-escaping="yes" select="Educator_Text"/></xsl:variable>
                  <xsl:if test="$text!=''">
                    <tr>
                      <td class="unit" style="border:1px solid #cccccc;"><xsl:value-of select="Educator_Text"/></td>
                    </tr>
                  </xsl:if>
                  </xsl:for-each>
                  </tbody>
                ...
                <table cellspacing="1" class="tablesorter table-3" style='width: 100%;'>
                  ...
                  <tbody>
                  <xsl:for-each select="EditPasport/Infosec/Educator_DocumentsList/item">
